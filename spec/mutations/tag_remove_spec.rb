...
RSpec.describe "Tag Remove", type: :request do
  include AuthHelper
  include JsonHelper

  def query
    %(
      mutation TagRemove($tagId: ID!, $songIds: [ID!]!) {
        tagRemove(input:{
          tagId: $tagId,
          songIds: $songIds
        }) {
  ...
  describe "adding an association" do
    it "associates new songs to the tag, noops on songs that already have the tag" do
      ...
      expect(song1.tags).not_to include(tag)
      expect(song2.tags).not_to include(tag)
      expect(song3.tags).not_to include(tag)
    end
  end
  ...
  context "when tag is misconfigured" do
    it "returns an error when tag does not exist" do
      ...
      expect(json_body.dig(:data, :tagRemove, :tag)).to be_nil
      expect(json_body.dig(:data, :tagRemove, :errors)).to include("Tag must be present")
    end
